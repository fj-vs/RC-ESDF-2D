...
- 新增 DDR-opt 风格前后端 + 控制链路：
  ...
  - 后端：`MSPlanner`（B 样条优化）结合 ESDF 梯度做平滑优化输出连续路径（`planned_path_opt`）
  - 控制：`MpcController` 根据优化路径滚动发布 `cmd_vel`（`geometry_msgs::Twist`）
...
新增可调参数：`search_safe_distance`、`allow_diagonal`（前端 JPS）、`w_ref`（后端 MS 参考路径跟踪权重）以及 `mpc_lookahead_dist`、`mpc_max_linear_vel`、`mpc_max_angular_vel`、`mpc_k_theta`（MPC 控制输出）。
