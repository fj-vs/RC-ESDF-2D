...
DdrEsdfPlanner::DdrEsdfPlanner() : cfg_(Config{}), backend_(cfg_.backend_cfg) {}
DdrEsdfPlanner::DdrEsdfPlanner(const Config& cfg) : cfg_(cfg), backend_(cfg.backend_cfg) {}
...
std::vector<double> BsplineEsdfPlanner::basisAt(double u, int num_ctrl, int degree, const std::vector<double>& knots) const {
    ...
    for (int i = 0; i < num_ctrl; ++i) {
        if ((u >= knots[i] && u < knots[i + 1]) ||
            (u == 1.0 && i == num_ctrl - 1 && u >= knots[i] && u <= knots[i + 1])) n[i] = 1.0;
    }
...
std::vector<Eigen::Vector2d> BsplineEsdfPlanner::optimize(
    ...
    const int degree = 3;
    const int n = static_cast<int>(init_ctrl_pts.size());
    std::vector<Eigen::Vector2d> ctrl_pts = init_ctrl_pts;
    ...
    for (int iter = 0; iter < cfg_.max_iterations; ++iter) {
        ...
        for (int s = 0; s < sample_count; ++s) {
            ...
            if (reference_path && !reference_path->empty()) {
...
std::vector<Eigen::Vector2d> BsplineEsdfPlanner::plan(
    ...
    return optimize(ctrl_pts, nullptr, start, goal, map, sampled_path);
}

std::vector<Eigen::Vector2d> BsplineEsdfPlanner::planFromReference(
    const std::vector<Eigen::Vector2d>& reference_path,
    const Eigen::Vector2d& start,
    const Eigen::Vector2d& goal,
    const GlobalEsdfMap& map,
    ...
    ctrl_pts.front() = start;
    ctrl_pts.back() = goal;
    return optimize(ctrl_pts, &reference_path, start, goal, map, sampled_path);
}

bool DdrEsdfPlanner::frontEndAstar(
    const Eigen::Vector2d& start,
    const Eigen::Vector2d& goal,
    const GlobalEsdfMap& map,
    ...
    auto toWorld = [&map](int x, int y) {
        return Eigen::Vector2d(map.origin().x() + (x + 0.5) * map.resolution(),
                               map.origin().y() + (y + 0.5) * map.resolution());
    };
    ...
    auto traversable = [&](int x, int y) {
        ...
        if (id < 0 || id >= static_cast<int>(esdf.size())) return false;
        return esdf[id] > cfg_.search_safe_distance;
    };
...
bool DdrEsdfPlanner::plan(
    const Eigen::Vector2d& start,
    const Eigen::Vector2d& goal,
    const GlobalEsdfMap& map,
    std::vector<Eigen::Vector2d>& front_end_path,
    std::vector<Eigen::Vector2d>& optimized_path) const {
    if (!frontEndAstar(start, goal, map, front_end_path)) return false;
    backend_.planFromReference(front_end_path, start, goal, map, &optimized_path);
